...
public interface PrizeTypeMapper {
     ...
     * @return
     */
    @Select("SELECT id, prize_name, prize_sum, remain_number, probability FROM prize_type ORDER BY probability ASC")
    List<PrizeTypeEntity> getPrize();

    @Update("UPDATE prize_type SET remain_number = #{entity.remainNumber} where id = #{entity.id}")
    int modify(@Param("entity") PrizeTypeEntity entity);
}
